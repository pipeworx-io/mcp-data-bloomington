...
const BASE = 'https://data.bloomington.in.gov';
const UA = 'pipeworx-mcp-data-bloomington/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'datasets',
    description: 'Search the Bloomington Open Data catalog of open datasets by keyword. Returns each dataset\'s resource_id, name, description, category and update date — pass the resource_id to query/metadata.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'query',
    description: 'Run a Socrata SoQL query against a Bloomington Open Data dataset by resource_id (e.g. "yv82-z42g"). Filter with where/select/group/order (SoQL clauses, without the leading $) plus limit/offset. Returns matching rows as JSON.',
    inputSchema: {
      type: 'object',
      properties: {
        resource_id: { type: 'string', description: 'Dataset id, e.g. "yv82-z42g" (from datasets).' },
  ...
  {
    name: 'metadata',
    description: 'Get a Bloomington Open Data dataset\'s schema + metadata (columns, types, row count, category, last-updated) by resource_id, e.g. "yv82-z42g".',
    inputSchema: {
      type: 'object',
      properties: { resource_id: { type: 'string', description: 'Dataset id, e.g. "yv82-z42g".' } },
      required: ['resource_id'],
    },
  },
];
  ...
  switch (name) {
    case 'datasets': {
      const p = new URLSearchParams({
        ...
        domains: 'data.bloomington.in.gov',
      });
    ...
    case 'query': {
      const id = reqStr(args, 'resource_id', '"yv82-z42g"');
      const p = new URLSearchParams();
    ...
    case 'metadata':
      return socrataGet(`/api/views/${reqStr(args, 'resource_id', '"yv82-z42g"')}.json`);
    default:
...
async function socrataGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`data.bloomington.in.gov: ${res.status}`);
  return res.json();
}
